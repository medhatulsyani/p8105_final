...
# We anticipate that some AED location data will be missing or have inaccurate coordinates. Furthermore, matching the specific point AED locations to traffic intersections which cover a larger area will be a challenge. When creating a dashboard with a large set of AED locations and traffic intersections can lead to lazy loading. We can also anticipate that ANOVA test will assume AED locations and traffic intersections are same across all five boroughs. Overall, we will prioritize efficient code and clear documentation to ensure reproducibility.
